package corecs.graph.shortestpath.undirected;
...
// BFS finds shortest Path in Any Graph
public class ShortestPathFinderInUnDirectedGraph {
 ...
 */
	...
	public static void main(String[] args) {
		...
		UnDirectedGraph graph = getUndircetedGraph();
		
		Integer fromVertex = 0;
		Integer toVertex = 2;
		
		String shortestPath = findShortestPath(fromVertex, toVertex, graph);
		
		System.out.println("Shortest Distance from Vertex " + fromVertex + " to " + toVertex + " is -> " + (shortestPath.length() -1));
		System.out.println("Shortest Path from Vertex " + fromVertex + " to " + toVertex + " is " + shortestPath );
		
	}
	
	private static UnDirectedGraph getUndircetedGraph() {
	...
	public static String findShortestPath(Integer fromVertex, Integer toVertex, UnDirectedGraph graph) {
		...
		visitedVertexList.add(fromVertex);
		...
		while(!queue.isEmpty()) {
			...
			for(Integer curNeighbourVertex : graph.getVertexToAdjListMap().get(curVertex)) {
				...
				if(!visitedVertexList.contains(curNeighbourVertex)) {
					...
					if(curNeighbourVertex.equals(toVertex)) {
						System.out.println("Meet toVertex -> " + toVertex);
						queue.clear();
						break;
					}
				}
		...
		shortestPath = getPath(toVertex, vertexParentMap); 
		
		return shortestPath;
	}
	
	private static String getPath(Integer vertex, Map<Integer, Integer> vertexParentMap) {
	
		StringBuilder path = new StringBuilder();
		
		path.append(vertex);
		Integer curVertex = vertexParentMap.get(vertex);

		while(curVertex != null) {
			path.append(curVertex);
			curVertex = vertexParentMap.get(curVertex);
		}
