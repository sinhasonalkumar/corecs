package corecs.graph.shortestpath.undirected.distance;
...
public class ShortestDistanceFinderInUnDirectedGraph {
 ...
 */
	...
	public static void main(String[] args) {
		...
		Integer shortestDistance = findShortestDistance(fromVertex, toVertex, graph);
		
		System.out.println("Shortest Distance from Vertex " + fromVertex + " to " + toVertex + " is of length -> " + shortestDistance);
	...
	public static Integer findShortestDistance(Integer fromVertex, Integer toVertex, UnDirectedGraph graph) {
		
		Integer shortestDistance = null;
		
		List<Integer> visitedVertexList = new ArrayList<Integer>(graph.getNoOfVertices());
		Map<Integer, Integer> distanceOfVertex = new HashMap<Integer, Integer>(graph.getNoOfVertices());
		
		Queue<Integer> queue = new LinkedList<Integer>();
		queue.add(fromVertex);
		visitedVertexList.add(fromVertex);
		distanceOfVertex.put(fromVertex, 0);
		...
		while(!queue.isEmpty()) {
			...
			for(Integer curNeighbourVertex : graph.getVertexToAdjListMap().get(curVertex)) {
				...
				if(!visitedVertexList.contains(curNeighbourVertex)) {
					...
					distanceOfVertex.put(curNeighbourVertex, distanceOfVertex.get(curVertex)+1);
		...
		shortestDistance = distanceOfVertex.get(toVertex); 
