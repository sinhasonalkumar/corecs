...
public class AllPossibleWordsInMatrix {

	
	private static int M, N;

	// All Neighbors template
	...
	public static void launchDFS(char[][] board, Set<String> words, boolean[][] processed, int i, int j, String path) {
		...
		for (int k = 0; k < 8; k++) {
			...
			if (isSafe(i + row[k], j + col[k], processed)) {
				launchDFS(board, words, processed, i + row[k], j + col[k], path);
			}
		}
	...
	// Function to search for given set of words in a matrix
	public static void searchInMatrix(char[][] board, List<String> input) {
		...
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < N; j++) {
				...
				launchDFS(board, words, processed, i, j, "");
			}
		}
